...
$num_instances_ingest   = 3
$instance_name_prefix   = "elasticsearch"
$vm_memory              = 3072
$vm_cpus                = 2
$subnet                 = "10.0.3.12" # For 10.0.3.12X
    ...
    config.vm.define vm_name = "%s%02d" % [$instance_name_prefix, i] do |server|
      ...
      if i == $num_instances
        config.vm.provision "ansible" do |ansible|
          ...
          ansible.groups = {
            "elastic" => ["#{$instance_name_prefix}0[1:#{$num_instances}]"],
            "elastic-master" => ["#{$instance_name_prefix}0[1:#{$num_instances_master}]"],
            "elastic-data" => ["#{$instance_name_prefix}0[1:#{$num_instances_data}]"],
            "elastic-ingest" => ["#{$instance_name_prefix}0[1:#{$num_instances_ingest}]"]
          }
        end
      end
    end
